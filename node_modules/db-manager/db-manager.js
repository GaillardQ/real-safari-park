...
var db_checkPokemonInArea = function(fb_id, coords, zone_length, nb_pokemon_zone, _callback)
{
    if(client != null)
    {
        ...
        var query = "SELECT pokemon.id as id, pokemon.name as name, pokemon.number as number, pokemon.gif as gif, pokemon.png as png, pokemon_category.name as categorie, pokemon_popped.popped_place as place, pokemon_popped.expired_at as expires_at "+
                    ...
                    "AND ST_distance(pokemon_popped.popped_place::geometry,ST_GeomFromEWKT('SRID=4326;POINT("+coords.lat+" "+coords.long+")')) < "+zone_length+
                    " AND pokemon_popped.catched=true "+
...
var db_savePoppedPokemons = function(area_pokemon, _callback)
{
        ...
        if(debug == true)
	{
        ...
        for(var i=0; i<area_pokemon.length; i++)
        {
            ...
            // ST_GeomFromText('POINT("+p.coords.k+" "+p.coords.B+")', 4326)
            query += "("+p.id+", POINT("+p.coords.k+" "+p.coords.B+"), 0, false),";
        }
