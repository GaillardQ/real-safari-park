...
var db_checkPokemonInArea = function(fb_id, coords, zone_length, nb_pokemon_zone, _callback)
{
    try
    {
        if(client != null)
        {
            ...
            var query = "SELECT pokemon.id as id, pokemon.name as name, pokemon.number as number, pokemon.gif as gif, pokemon.png as png, pokemon_category.name as category, "+
                                " pokemon_popped.popped_place as place, pokemon_popped.expired_at as expires_at, "+
                                " ST_distance_sphere(ST_SetSRID(pokemon_popped.popped_place::geometry,4326),ST_GeomFromEWKT('SRID=4326;POINT("+coords.lat+" "+coords.long+")')) as d "+
