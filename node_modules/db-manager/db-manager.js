...
var db_createNewPokemons = function(pokemons, _callback)
{
    if(client != null)
    {
        ...
        var rarity;
        // On stosk le nombre de pokemon de chaque type dont nous avons besoin
        ...
        console.log(util.inspect(obj_rarity, false, null));
        
        var query = "SELECT p.name, p.number, p.png, p.gif, pc.name as rarity FROM pokemon as p, pokemon_category as pc WHERE p.id_category = pc.id AND pc.name IN ("+query_rarity+") ORDER BY pc.name ASC, p.name ASC";
        client.query(query, function(err, result) {
            if (err) {
            ...
            keys.forEach(function (k, i) {
                ...
                for(var i=0; i<nb; i++)
                {
                    for(var p in pokemons)
                    {
                        ...
                        {
                            size = tab.length;
                            random = Math.floor(Math.random()*nb);
            ...
            console.log(util.inspect(ar_pokemon, false, null));
        });
